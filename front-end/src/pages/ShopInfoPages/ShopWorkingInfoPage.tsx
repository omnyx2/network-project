...
function ShopWorkingInfoPage() {
  ...
  const handleCreateTypeDate = () => {
    const initData = new DaysDataStateSerializer(undefined).createTypeOfData(
      daysDataState
    );
    setDaysDataState(initData);
  };
  ...
  return (
    <>
      ...
      <div>
        <Text color="#212529" fw={500} fz="md">
          Type1{" "}
        </Text>
        {WorkingDaysSelector({ handleChecked, daysData: daysDataState })}
        <div>
          <div>
            <Text color="#212529" fw={500} fz="sm">
              비정기 휴업일 설정{" "}
            </Text>
            <TimePicker />
          </div>

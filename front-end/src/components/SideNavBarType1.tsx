...
export function DoubleNavbar() {
  ...
  const mainLinks = mainLinksMockdata.map((navItem) => (
    <Tooltip label={navItem.label} position="right" withArrow transitionDuration={0} key={navItem.label}>
      <UnstyledButton
        onClick={() => { setActive(navItem.label); setActiveItem(navItem)}}
        className={cx(classes.mainLink, { [classes.mainLinkActive]: navItem.label === active })}
      >
        <Link to={`${baseUrl}${navItem.link}`} className="link"> 
